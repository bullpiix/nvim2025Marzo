...
function _G.close_input_commit()
    _G.execute_command(string.format("git commit -m '%s'", nameRama), 1)
    vim.api.nvim_win_close(btn_win_id, true)
...
function _G.close_input_add()
    ...
    if input_text == "done" then
        if #files_to_add > 0 then
            ...
            _G.execute_command(string.format("git add %s", all_files), 1)
...
function _G.close_input_rama()
    local input_text = vim.api.nvim_buf_get_lines(input_buf_id, 0, -1, false)[1]
    _G.execute_command(string.format("git checkout -b %s", input_text), 1)
    nameRama = input_text
    vim.api.nvim_win_close(btn_win_id, true)
...
function _G.close_input_link()
    local input_text = vim.api.nvim_buf_get_lines(input_buf_id, 0, -1, false)[1]
    _G.execute_command(string.format("git remote add origin %s.git", input_text), 1)
    vim.api.nvim_win_close(btn_win_id, true)
...
vim.api.nvim_set_keymap("n", "<Space>e", ":lua _G.show_input_interface()<CR>", { noremap = true, silent = true })
