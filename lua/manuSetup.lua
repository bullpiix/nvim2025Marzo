...
vim.api.nvim_set_keymap('n', 'u', '<Left>', { noremap = true, silent = true })
vim.api.nvim_set_keymap('n', 'i', '<Right>', { noremap = true, silent = true })
vim.api.nvim_set_keymap('n', 'p', 'b', { noremap = true, silent = true })
vim.api.nvim_set_keymap('n', 'o', 'w', { noremap = true, silent = true })
...
vim.api.nvim_set_keymap('n', '}', [[:normal! cgn<C-r>+<Esc>]], { noremap = true, silent = true })
...
function WrapWithHtmlTag()
  local word = vim.fn.expand("<cword>") -- palabra bajo el cursor
  local row, col = unpack(vim.api.nvim_win_get_cursor(0)) -- posición del cursor
  local line = vim.api.nvim_get_current_line()

  -- Busca inicio de la palabra en la línea
  local s, e = line:find(word, col - #word + 1, true)
  if not s then return end

  -- Construye nueva línea
  local new_line = line:sub(1, s - 1) .. "<" .. word .. "></" .. word .. ">" .. line:sub(e + 1)
  vim.api.nvim_set_current_line(new_line)

  -- Mueve el cursor al centro de las etiquetas
  local new_col = s + #word + 1 -- después de <word>
  vim.api.nvim_win_set_cursor(0, {row, new_col})
end
vim.api.nvim_set_keymap('n', '+', ':lua WrapWithHtmlTag()<CR>', { noremap = true, silent = true })
