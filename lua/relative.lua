...
vim.api.nvim_set_keymap("n", "b", ":lua _G.input_relativo()<CR>" ,{ noremap = true, silent= true})


function _G.input_number()
    input_var1 = vim.api.nvim_create_buf(false, true)
    ...
    vim.api.nvim_buf_set_keymap(input_var1,"i","p","8",{ noremap = true, silent = true})
    vim.api.nvim_buf_set_keymap(input_var1,"i","l","9",{ noremap = true, silent = true})
    vim.api.nvim_buf_set_keymap(input_var1,"i","b","0",{ noremap = true, silent = true})
...
function _G.type_number()
    local line = vim.api.nvim_get_current_line()
    local input = line:gsub("^Buscar:%s*", "")  -- lo que el usuario escribió

    -- guarda el buffer actual (archivo que estabas editando)
    ...
    if input ~= "" then
        -- Inserta en el buffer del archivo
...
vim.api.nvim_set_keymap("i", "<Up>", "<Esc>:lua _G.input_number()<CR>" ,{ noremap = true, silent= true})
